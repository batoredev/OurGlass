...
```
Read INSTALL.md at <path-to-pack> and execute it fully. Don't ask me to confirm steps.
```

`INSTALL.md` is the autonomous runbook: it detects your stack, adapts the ownership map and
permissions to it, merges rather than overwrites, verifies the hooks actually fire, and reports
what still needs you. It is idempotent — safe to re-run.

The manual sequence below is the fallback if you'd rather drive it yourself.
