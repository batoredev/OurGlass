...
# PHASE 5 — DEEP SPECIALISTS (22–31)

Spawned when the request has a dominant specialty. Not additions to every team — replacements for the generalist when the signal is clear. See `docs/ROUTER.md`.

| # | Agent | Writes | Owns | Signature skills |
|---|---|---|---|---|
| 22 | Design Engineer | yes | Components, tokens-in-code | `ui-skills`, `ui.live`, `animos`, `ui-ux-pro-max` |
| 23 | Motion Engineer | yes | Motion glob | `motion`, `motion-dom`, `motion-utils`, `framer-motion`, `animejs`, Emil Kowalski |
| 24 | Taste Director | advises | Direction docs | `taste`, `awesome design`, `impeccable` |
| 25 | Web Standards Engineer | **no** | — | `vercel web guidelines` |
| 26 | Accessibility Engineer | **no** | — | `ui-ux-pro-max` a11y, Emil Kowalski (focus) |
| 27 | E2E Automation Engineer | yes | `e2e/**` | `playwright`, `playwright-core` |
| 28 | Mobile QA Engineer | yes | Mobile test glob | Maestro, Detox |
| 29 | API Contract Engineer | yes | Schema / contract glob | OpenAPI, protobuf, GraphQL |
| 30 | Research Analyst | **no** | — | Web search, primary sources |
| 31 | Technical Writer (DX) | docs | Developer docs | `/document-generate`, `vercel web guidelines` |

**Three distinctions worth internalising:**

**23 vs 07.** Motion is where AI-generated UI most obviously fails — absent, or everywhere at 300ms ease-in-out. Agent 23 carries duration-follows-distance, interruptibility, easing semantics, and `prefers-reduced-motion` as a real alternative rather than `animation: none`.

**27 vs 12.** Agent 12 drives the gstack browse daemon: one shared, stateful, exploratory session, exclusive to it. Agent 27 drives Playwright: isolated contexts per run, code-defined, identical every time. **No conflict — they can share a mission.** Agent 27 is also the WAT-aligned half, since its output is deterministic scripts rather than reasoning.

**28 exists because 12 cannot reach a React Native app.** On Expo projects the entire browser roster is inapplicable. Without agent 28 a mobile project has no QA agent at all.

**30 is the capability-gap fallback.** When no agent and no skill covers a request, it researches from primary sources and labels verified versus inferred. An honest "could not determine" is a valid output; a fabricated API signature is not.
...
| 22 | Design Engineer | yes | no | `/freeze` | `ui-skills`, `ui.live` |
| 23 | Motion Engineer | yes | no | `/freeze` | `motion`, Emil Kowalski |
| 24 | Taste Director | advises | no | — | `taste` |
| 25 | Web Standards Engineer | **no** | via 12 | — | `vercel web guidelines` |
| 26 | Accessibility Engineer | **no** | via 12 | — | WCAG 2.2 |
| 27 | E2E Automation Engineer | yes | **own Playwright** | `/freeze e2e/` | `playwright` |
| 28 | Mobile QA Engineer | yes | n/a | `/freeze` | Maestro, Detox |
| 29 | API Contract Engineer | yes | no | `/freeze` | OpenAPI |
| 30 | Research Analyst | **no** | no | — | primary-source research |
| 31 | Technical Writer (DX) | docs | no | `/freeze` | `/document-generate` |
